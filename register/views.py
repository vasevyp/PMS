...
def index(request):
    summ_toorder = ToOrder.objects.aggregate(
        sum_order=Sum('order_sum')).get('sum_order')
    summ_toorder_3 = ToOrder_3.objects.aggregate(
        sum_order=Sum('order_sum')).get('sum_order')
    if summ_toorder:
    ...
    stock_actual_cost = StockItem.objects.aggregate(
        sum_order=Sum('actual_cost')).get('sum_order')
    summa_stock = stock_actual_cost
    last_order = LastOrder.objects.all()
    summ_lastorder = LastOrder.objects.aggregate(
        sum_order=Sum('order_cost')).get('sum_order')
    ...
    summ_delivery = StockItem.objects.exclude(delivery=0).aggregate(
        sum_order=Sum('delivery_cost')).get('sum_order')

    # Среднесуточные продажи
    last_date = SaleProduct.objects.latest(
        'date').date  # дата последней продажи

    sales7 = SaleProduct.objects.filter(date__range=(
        last_date-timedelta(days=7), last_date)).aggregate(sum_revenue=Sum('revenue')).get('sum_revenue')
    weeksales = sales7/7/1000
    sales7_7 = SaleProduct.objects.filter(date__range=(
        last_date-timedelta(days=14), last_date-timedelta(days=8))).aggregate(sum_revenue=Sum('revenue')).get('sum_revenue')
    weeksales_7 = sales7_7/7/1000
    pr_week = (weeksales/weeksales_7-1)*100
    sales30 = SaleProduct.objects.filter(date__range=(
        last_date-timedelta(days=30), last_date)).aggregate(sum_revenue=Sum('revenue')).get('sum_revenue')
    monthsales = sales30/30/1000
    sales30_30 = SaleProduct.objects.filter(date__range=(
        last_date-timedelta(days=60), last_date-timedelta(days=31))).aggregate(sum_revenue=Sum('revenue')).get('sum_revenue')
    monthsales_30 = sales30_30/30/1000
    pr_month = (monthsales/monthsales_30-1)*100

    daily_s = StockItem.objects.aggregate(
        s=Sum(F('last_cost')*F('daily_requirement')))['s']
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            StockItem.objects.create(code=code, name=name, slug=do_slug(name), unit=unit, unit_cost=unit_cost,
                                     first_cost=unit_cost, last_cost=unit_cost, delivery_time=delivery_time, supplier=supplier)
            form.save()
...
def add_to_recipe(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            RecipeIngredient.objects.get_or_create(product=product, ingredient=ingredient, code=code,
                                                   code_ingr=code_ingr, unit=unit, unit_cost=unit_cost, ratio=ratio, ingredient_cost=unit_cost*ratio)
